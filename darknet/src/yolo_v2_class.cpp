    ...
    : cur_gpu_id(gpu_id)
{
    ...
    for (j = 0; j < NFRAMES; ++j)
        detector_gpu.predictions[j] = (float *)calloc(l.outputs, sizeof(float));
    for (j = 0; j < NFRAMES; ++j)
        detector_gpu.images[j] = make_image(1, 1, 3);
...
LIB_API Detector::~Detector()
{
    ...
    for (int j = 0; j < NFRAMES; ++j)
        free(detector_gpu.predictions[j]);
    for (int j = 0; j < NFRAMES; ++j)
...
#ifdef OPENCV
LIB_API std::vector<bbox_t> Detector::detect_cv(cv::Mat mat, float thresh, bool use_mean)
{
    if (mat.data == NULL)
        throw std::runtime_error("Image is empty");
    std::shared_ptr<image_t> img = mat_to_image(mat);
    return detect(*img, thresh, use_mean);
}
#endif // OPENCV
